...
static HRESULT appendStrW(CQuickBytes *out, const WCHAR *str)
{
    ...
    SIZE_T len = PAL_wcslen(str) * sizeof(WCHAR);
    SIZE_T oldSize = out->Size();
    ...
    IMetaDataImport *pIMDI)  // ptr to IMDInternal class with ComSig
{
    ...
    case ELEMENT_TYPE_INTERNAL:
        ...
        typePtr += sizeof(void *);
        WCHAR tempBuffer[64];
        swprintf_s(tempBuffer, 64, W("pMT: %p"), pMT);
        appendStrW(out, tempBuffer);
        break;
    ...
    case ELEMENT_TYPE_CMOD_REQD:
        str = W("required_modifier ");
        goto DO_CLASS;

    case ELEMENT_TYPE_CMOD_OPT:
        str = W("optional_modifier ");
        goto DO_CLASS;
...
__checkReturn static HRESULT PrettyPrintTypeA(
    PCCOR_SIGNATURE &typePtr, // type to convert,
    size_t typeLen,           // Maximum length of the type.
    CQuickBytes *out,         // where to put the pretty printed string
    IMDInternalImport *pIMDI, // ptr to IMDInternal class with ComSig
    IMetaDataTables *pIMDTI)  // ptr to IMetaDataTables class with ComSig

{
    ...
    DO_CLASS:
        ...
        if (TypeFromToken(tk) == mdtTypeSpec)
        {
            ULONG cSig;
            PCCOR_SIGNATURE sig;
            IfFailGo(pIMDI->GetSigFromToken(tk, &sig, &cSig));
            IfFailGo(PrettyPrintTypeA(sig, cSig, out, pIMDI, pIMDTI));
        }
        else
        {
            if (TypeFromToken(tk) == mdtTypeRef)
            {
                ...
                if (FAILED(GetNameOfTypeRef(pIMDTI, tk, &pNS, &pN)))
                {
            ...
            else
            {
                ...
                if (FAILED(GetNameOfTypeDef(pIMDTI, tk, &pN, &pNS)))
                {
    ...
    case ELEMENT_TYPE_SZARRAY:
        IfFailGo(PrettyPrintTypeA(typePtr, (typeEnd - typePtr), out, pIMDI, pIMDTI));
    ...
    case ELEMENT_TYPE_FNPTR:
        ...
        IfFailGo(PrettyPrintSigWorkerInternal(typePtr, (typeEnd - typePtr), "", out, pIMDI, pIMDTI));
        break;
    ...
    IMetaDataTables *pIMDTI)
{
    ...
    return PrettyPrintSigWorkerInternal(typePtr, typeLen, name, out, pIMDI, pIMDTI);
} // HRESULT PrettyPrintSigInternalLegacy()
...
HRESULT PrettyPrintSigWorkerInternal(
    ...
    CQuickBytes *out,         // where to put the pretty printed string
    IMDInternalImport *pIMDI, // Import api to use.
    IMetaDataTables *pIMDTI)  // Import api to use.
